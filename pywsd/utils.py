...
def lemmatize(ambiguous_word, pos=None, neverstem=False,
              lemmatizer=wnl, stemmer=porter, postagger=pos_tag):
    """
    ...
    if not wn.synsets(lemma) and "_" in ambiguous_word:
        lemma = []
        for word in ambiguous_word.split("_"):
            pos_word = postagger([word])
            if pos_word:
                pos_word = penn2morphy(pos_word[0][1])
            if pos_word in ['a', 's', 'r', 'n', 'v']:
                word = lemmatizer.lemmatize(word, pos=pos_word)
            lemma.append(word)
        lemma = "_".join(lemma)
...
def lemmatize_sentence(sentence, neverstem=False, keepWordPOS=False,
    ...
    for word, pos in postagger(sentence):
        pos = penn2morphy(pos)
        lemma = lemmatize(word.lower(), pos, neverstem,
                  lemmatizer, stemmer, postagger)
        lemmas.append(lemma)
        if len(wn.synsets(lemma)) == 1:
            pos = wn.synsets(lemma)[0].pos()
        poss.append(pos)
        words.append(word)
    if keepWordPOS:
